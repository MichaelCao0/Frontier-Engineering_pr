...
def evaluate(program_path: str, *, repo_root: Path | None = None):
    ...
    benchmark_dir = (
        repo_root / "benchmarks" / "Robotics" / "BasicArmInverseKinematics"
    ).resolve()
    if not benchmark_dir.is_dir():
        benchmark_dir = (repo_root / "Robotics" / "BasicArmInverseKinematics").resolve()
    ...
    work_dir = Path(tempfile.mkdtemp(prefix="fe_ik_")).resolve()
    try:
        sandbox_task = (work_dir / "BasicArmInverseKinematics").resolve()
        ...
        spec = importlib.util.spec_from_file_location("fe_basic_ik_eval", eval_path)
        ...
        if feasible:
            ...
            metrics["valid"] = 1.0
            metrics["ik_score"] = raw_score
            metrics["combined_score"] = raw_score / 1e6
        else:
            artifacts["error_message"] = "infeasible IK submission"
